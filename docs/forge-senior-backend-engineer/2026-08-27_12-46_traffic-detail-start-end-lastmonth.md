# Traffic detail_start/detail_end range + last-month default

- **Date:** 2026-08-27 12:46
- **Scope:**
  - `app/Support/Period.php` — added `lastMonth()` (rolling trailing month) and `detailStart()`/`detailEnd()` (`d.m.Y`, mirrors `paymentsStart()`/`paymentsEnd()`); updated class docblock.
  - `app/Services/Sola/SolaClient.php` — `trafficDetail()` signature changed from `(accountId, month)` to `(accountId, begin, end)`; sends `detail_start`/`detail_end` instead of `detail_month`.
  - `app/Services/Sola/FakeSolaServer.php` — local dev stub: renamed `fromPayDate()` → `fromDmyDate()` (now shared by payments and traffic), `trafficDetail()` reads `detail_start`/`detail_end`, removed now-unused `daysOf()`.
...
- **User must do:** Nothing (no migrations/env vars). Recommend watching the first live `/statistics` page loads after deploy to confirm SOLA actually honors `detail_start`/`detail_end` given the unverified-contract risk above.
